...
@pytest.mark.parametrize(
    "redirect_uri, valid",
    [("http://test/callback", True), ("http://test/invalid", False)],
)
...
def test_get_default_scopes(request_):
    ...
    assert auth_client().default_scopes == validator.get_default_scopes(
        test_client.client_id, request_
    )


@pytest.mark.parametrize("response_type, valid", [("code", True), ("not_code", False)])
def test_validate_response_type(request_, response_type, valid):
    ...
    assert (
        validator.validate_response_type(
            test_client.client_id, response_type, test_client, request_
        )
        == valid
    )
...
def test_save_authorization_code(cache):
    ...
    validator.save_authorization_code(
        request.client.client_id, {"code": "code"}, request
    )

    assert cache.get("code")


@pytest.mark.parametrize("param", [("client_id", None), ("client_id", "not_in_the_db")])
...
@pytest.mark.parametrize(
    "param, client_secret, authenticated",
    [
...
@pytest.mark.parametrize(
    "cache_entry, client_id, validated",
    [
...
def test_validate_code_cache(request_, mocker, cache_entry, client_id, validated):
    ...
    assert (
        validator.validate_code(client_id, "a_code", test_client, request_) == validated
    )


@pytest.mark.parametrize(
    "cache_entry, redirect_uri",
    [
...
def test_confirm_redirect_uri(mocker, cache_entry, redirect_uri):
    ...
    assert (
        validator.confirm_redirect_uri(
            test_client.client_id, "a_code", test_client, redirect_uri
        )
        == False
    )
...
@pytest.mark.parametrize(
    ...
    [(auth_client().grant_type, True), ("refresh_token", True), ("invalid", False)],
)
def test_validate_grant_type(request_, grant_type, supported):
    ...
    assert (
        validator.validate_grant_type(
            test_client.client_id, grant_type, test_client, request_
        )
        == supported
    )
...
@pytest.mark.parametrize(
    "now, expired", [(datetime(2016, 12, 1), True), (datetime(2016, 10, 1), False)]
)
...
def test_validate_refresh_token_no_refresh_token(request_, mocker):
    ...
    assert (
        validator.validate_refresh_token(refresh_token, test_client, request_) == False
    )
